...
class CartProvider extends ChangeNotifier {
  ...
  Future<void> getCartListProvider({required BuildContext context}) async {
    ...
    try {
      ...
      if (getData[ApiAndParams.status].toString() == "1") {
        ...
        gst = double.parse(cartData.data.gst);
        itemPrice = double.parse(cartData.data.itemsPrice);
        discount = double.parse(cartData.data.discount);
        savedAmount = double.parse(cartData.data.savedDiscount);
  ...
  setItemPrice(double price) {
    itemPrice = price;
    notifyListeners();
  }

  setGst(double newgst) {
    gst = newgst;
    notifyListeners();
  }
  ...
  setSavedAmount(double newSavedAmount) {
